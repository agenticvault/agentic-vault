 ...
 *
 * Exercises the complete DeFi pipeline:
 *   1. WETH wrap (ETH -> WETH deposit)
 *   2. ERC-20 approve (WETH -> Uniswap Router)
 *   3. Uniswap V3 exactInputSingle (WETH -> USDC)
 *   4. Verify USDC received
 *   5. ERC-20 approve (WETH -> Aave V3 Pool)
 *   6. Aave V3 supply (WETH)
 *   7. Verify supply success
 *
 ...
 *       when the signer address has insufficient balance, when
 *       the Uniswap V3 WETH/USDC pool has no liquidity, or when
 *       the Aave V3 Pool contract is not deployed.
 */
...
const AAVE_V3_POOL = '0x6Ae43d3271ff6888e7Fc43Fd7321a503ff738951' as Address;
...
describeWithTestnet('Sepolia DeFi Full Flow (Wrap → Swap → Supply)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.warn(
        `\n  RPC connection failed (${err instanceof Error ? err.message : String(err)}).` +
          '\n  Set SEPOLIA_RPC_URL — skipping DeFi flow tests.\n',
      );
      suiteSkipped = true;
      return;
    }
    ...
    try {
      ...
      if (/token.*expired|credentials.*expired|sso.*login/i.test(msg)) {
        console.warn(
          '\n  AWS credentials expired — skipping DeFi flow tests.\n' +
    ...
    if (balance < MIN_BALANCE_WEI) {
      console.warn(
        `\n  Signer ${signerAddress} has ${formatEther(balance)} ETH (need >= ${formatEther(MIN_BALANCE_WEI)}).` +
          '\n  Fund via faucet: https://sepoliafaucet.com/ — skipping DeFi flow tests.\n',
      );
      suiteSkipped = true;
      return;
    }
    ...
    if (!selectedFee) {
      console.warn('\n  No WETH/USDC pool with liquidity on Sepolia — skipping DeFi flow tests.\n');
      suiteSkipped = true;
      return;
    }

    // --- Check Aave V3 Pool contract is deployed ---
    try {
      const code = await publicClient.getCode({ address: AAVE_V3_POOL });
      if (!code || code === '0x') {
        console.warn('\n  Aave V3 Pool not deployed on Sepolia — skipping DeFi flow tests.\n');
        suiteSkipped = true;
        return;
      }
    } catch {
      console.warn('\n  Failed to check Aave V3 Pool — skipping DeFi flow tests.\n');
      suiteSkipped = true;
      return;
    }
  }, 60_000);
  ...
  let approveRouterTxHash: `0x${string}`;
  ...
  it('should approve WETH for Uniswap V3 Router', async () => {
    ...
    approveRouterTxHash = result.txHash;
    expect(result.txHash).toMatch(/^0x[0-9a-f]{64}$/);
    expect(result.blockNumber).toBeGreaterThan(0n);
  ...
  let preSwapUsdcBalance: bigint;

  it('should swap WETH -> USDC via Uniswap V3 exactInputSingle', async () => {
    // Capture pre-swap USDC balance for delta-based verification
    preSwapUsdcBalance = await publicClient.readContract({
  ...
  it('should have received USDC after swap', async () => {
    ...
    expect(usdcBalance).toBeGreaterThan(preSwapUsdcBalance);
    console.log(`  USDC balance: ${formatUnits(usdcBalance, 6)} (+${formatUnits(usdcBalance - preSwapUsdcBalance, 6)})`);
  }, 10_000);

  // --------------------------------------------------------------------------
  // Step 5: Approve WETH for Aave V3 Pool
  // --------------------------------------------------------------------------

  let approveAaveTxHash: `0x${string}`;

  it('should approve WETH for Aave V3 Pool', async () => {
    const data = encodeFunctionData({
      abi: erc20Abi,
      functionName: 'approve',
      args: [AAVE_V3_POOL, SUPPLY_AMOUNT],
    });
    ...
    approveAaveTxHash = result.txHash;
    expect(result.txHash).toMatch(/^0x[0-9a-f]{64}$/);
    expect(result.blockNumber).toBeGreaterThan(0n);
  ...
  it('should supply WETH to Aave V3 Pool', async () => {
    const data = encodeFunctionData({
      abi: aaveV3SupplyAbi,
      functionName: 'supply',
      args: [WETH_SEPOLIA, SUPPLY_AMOUNT, signerAddress, 0],
    });
  ...
  it('should have completed supply successfully', () => {
    // signAndBroadcast waits for receipt with confirmations — reaching here
    // means the supply tx was mined and confirmed on-chain.
    expect(supplyTxHash).toMatch(/^0x[0-9a-f]{64}$/);
  });
  ...
  it('should print transaction summary', () => {
    console.log('\n  ========================================');
    console.log('       DEFI FULL FLOW TRANSACTIONS');
    console.log('  ========================================');
    if (wrapTxHash) console.log(`  1. WETH Wrap:         ${ETHERSCAN_BASE}${wrapTxHash}`);
    if (approveRouterTxHash)
      console.log(`  2. Approve (Router):  ${ETHERSCAN_BASE}${approveRouterTxHash}`);
    if (swapTxHash) console.log(`  3. WETH->USDC Swap:   ${ETHERSCAN_BASE}${swapTxHash}`);
    if (approveAaveTxHash)
      console.log(`  5. Approve (Aave):    ${ETHERSCAN_BASE}${approveAaveTxHash}`);
    if (supplyTxHash) console.log(`  6. Aave V3 Supply:    ${ETHERSCAN_BASE}${supplyTxHash}`);
    console.log('  ========================================\n');
