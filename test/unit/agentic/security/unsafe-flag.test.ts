...
const SAFE_TOOLS = ['get_address', 'health_check', 'sign_swap', 'sign_permit', 'sign_defi_call', 'get_balance', 'send_transfer', 'send_erc20_transfer'] as const;
...
describe('--unsafe-raw-sign flag', () => {
  ...
  describe('when unsafeRawSign is true', () => {
    ...
    it('should have exactly 10 tools registered', () => {
      const ctx = createMockContext();
      const server = createMcpServer({ ...ctx, unsafeRawSign: true });
      const tools = getRegisteredToolNames(server);
